...
//	acme_devp->rx_ring.status=acme_devp->rx_ring->status & 0xFE;
	msleep(500);
	//printk("(task! 0x%08X, ",readl(acme_devp->hw.led_addr));
	writel(LED_OFF,acme_devp->hw.led_addr);
	//printk("0x%08X )\n",readl(acme_devp->hw.led_addr));
...
static irqreturn_t acme_irq_handler(int irq, void *data){
	...
	case LSC_INT:
	case RXQ_INT:
		//printk("(interrupt ICR: 0x%08X\n)",cause);
		//printk("(int! 0x%08X, ",readl(acme_devp->hw.led_addr));
...
static int acme_open(struct inode *inode,struct file *filp){
	int err=0;
//	printk("(open! 0x%08X, ",readl(acme_devp->hw.led_addr));
	writel(LED_OFF,acme_devp->hw.led_addr);
//	printk("0x%08X )\n",readl(acme_devp->hw.led_addr));
	return err;
}
...
static int acme_close(struct inode *inode,struct file *filp){
	int err=0;
	//printk("(close! 0x%08X, ",readl(acme_devp->hw.led_addr));
	writel(LED_OFF,acme_devp->hw.led_addr);
	//printk("0x%08X )\n",readl(acme_devp->hw.led_addr));
	return err;
}
